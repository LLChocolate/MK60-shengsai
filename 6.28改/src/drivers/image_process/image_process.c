...
Image_lieData  Image_lie={{85,165,245},{0}};
Island_Data    Island={
                  .Correct_hang = 150,
                  .Stay_hang_use = 137,//这个行是瞎给的
                  .Image_Start_hang = 83,
                  .Next_Island_flag_delay_const = 1700,
...
u8 double_AD(void)
{
  if((L_AD_Ave>1000)&&(R_AD_Ave>1800)
     ||(L_AD_Ave>1800)&&(R_AD_Ave>1000))
  {
    return 1;
  }
  else 
...
u8 In_Island(void)
{
  ...
  else 
  {
    ...
    if(center == -1)//寻找失败或者已完全进入环岛
    {
      ...
      if(Island.In2Stay_cnt>=20)//连续10次寻找失败，认为进入环岛，更新状态
      {
    ...
    else
    {
      if(Island.State==Left_Island_pre)
      {
        ...
        else
        {
          ...
          if(Impulse_hang<95)
            center_use = ((center - (center - 319)*(Impulse_hang - Start_Point)*1.0/(Impulse_hang - Island.Correct_hang)) + 0)/2 + 60;//布线（三角形相似）
          else
            center_use = ((center - (center - 319)*(Impulse_hang - Start_Point)*1.0/(Impulse_hang - Island.Correct_hang)) + 0)/2 + 40;//布线（三角形相似）
          if(center_use<35)
            center_use = 35;
        }
      ...
      else if(Island.State==Right_Island_pre)
      {
        ...
        {
          Island.In2Stay_cnt = 0;//清零
          center_use = ((center - (center - 0)*(Impulse_hang - Start_Point)*1.0/(Impulse_hang - Island.Correct_hang)) + 319)/2 - 40;
          if(center_use>285)
            center_use = 285;
        }
        Island.In_Center = center_use;//保存上一次的中心点
...
u8 Out_Island(void)
{
  ...
  if(Island.State==Right_Island_out)//补线
  {
    ...
    if((center_impulse!=-1) && (center_use>Image_lie.Three_Lie[1]))//有效性检验
    {
...
//调试用，显示中心点
    ...
    else if(End_End<Start_Point-15)
    {
      get_black_line(Image_fire[Start_Point],Start_Point);//45cm处中心点
      if(Image_lie.Three_lie_end[0]>Image_hang.hang_use+5
         &&Image_lie.Three_lie_end[1]>Image_hang.hang_use+5
...
//        LCD_DrawBigPoint(Island.Out_Center,Start_Point,Magenta);//行列颠倒
  ...
  {
    center_use = ((center_impulse - (center_impulse - 319)*(End_End - Start_Point)*1.0/(End_End - Start_End)) + 0)/2+15;
    if((center_impulse!=-1)&&(center_use<Image_lie.Three_Lie[1]))//有效性检验
    {
//      Beep_Once(&Image_Island_Test_Beep);
      if(center_use < 35)
...
//调试用，显示中心点
  ...
  else if(Cross.State==L2Cross_True)
  {
    Cross.Cross_delay_flag = 1;
    Cross.Cross_delay_cnt = Cross.Cross_delay_cnt_const;
    center_use = ((center_impulse - (center_impulse - 319)*(End_End - Start_Point)*1.0/(End_End - Start_End)) + 0)/2 - 20;
    if(center_use<Image_lie.Three_Lie[1]-10)//有效性检验
    {
      if(center_use < 35)
        center_use = 35;
