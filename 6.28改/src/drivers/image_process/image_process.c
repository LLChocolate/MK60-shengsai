...
u8 double_AD(void)
{
  if((L_AD_Ave>1980)||(R_AD_Ave>2000))
  {
    return 1;
  }
  else 
...
u8 Out_Island(void)
{
  ...
  {
    center_use = ((center_impulse - (center_impulse - 0)*(End_End - Start_Point)*1.0/(End_End - Start_End)) + 319)/2-40;
...
{
  u8 Far_Lie[35];
  int Diff_Far_Lie[34];//一阶差分
  int DDiff_Far_Lie[33];//二阶差分
  int Liner_cnt  = 0;
  int out_center = -1;
  u8  Impulse_flag = 0;
